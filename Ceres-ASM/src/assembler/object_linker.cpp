...
namespace ceres::casm
{
	...
	std::optional<vm::Program> ObjectLinker::link(std::vector<ObjectArchive::Member> inputs, const ObjectLinkOptions& options)
	{
		_errors.clear();
		...
		const u32 textStart = vm::Memory::UnrestrictedSegmentStart.value();
		...
		for (usize i = 0; i < members.size(); ++i)
		{
			...
			for (const Relocation& relocation : object.relocations)
			{
				...
				if (position + vm::Instruction::Size > text.size())
				{
				...
				vm::Instruction instruction(
					static_cast<vm::Instruction::RawType>(text[position]) |
					(static_cast<vm::Instruction::RawType>(text[position + 1]) << 8) |
					(static_cast<vm::Instruction::RawType>(text[position + 2]) << 16) |
					(static_cast<vm::Instruction::RawType>(text[position + 3]) << 24));
				...
				const vm::Instruction::RawType raw = instruction.raw();
		...
		if (options.emitDebugInfo)
		{
			for (usize i = 0; i < members.size(); ++i)
			{
				...
				auto piece = debug::DebugInfo::deserialize(object.debugSection);
				if (!piece)
				{
		...
		return vm::Program::make(header, text, rodata, data, debugSection);
	}
}
