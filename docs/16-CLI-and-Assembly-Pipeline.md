...
| `ceres run <file.casm\|file.cres> [--memory <bytes>] [--disk <image>] [--window \| --terminal] [--port <n>=<image>]... [--cart <n>=<file>]... [--env <name>=<value>]... [--host-dir <dir>] [-- <argument>...]` | Runs a program, assembling it first if given a `.casm` source file. |
