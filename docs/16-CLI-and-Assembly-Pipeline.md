...
The binary lands in `Ceres/build/<preset>/bin/<config>/`. `--preset msvc` is the same on Windows;
the Visual Studio solution is generated by CMake rather than maintained by hand, so opening the
`Ceres/` folder in VS is enough. (`std::print` needs `-lstdc++exp` on MinGW, which the build
detects by asking the linker.) The tests run with `ctest --preset gcc-debug`.
...
| `ceres run <file.casm\|file.cres> [--memory <bytes>] [--disk <image>] [--window \| --terminal] [--strict-mmio] [--rtc <YYYY-MM-DDThh:mm:ss>] [--speed realtime\|max\|<f>x] [--cpu-clock <hz>] [--record <file> \| --replay <file>] [--port <n>=<image>]... [--cart <n>=<file>]... [--env <name>=<value>]... [--host-dir <dir>] [-- <argument>...]` | Runs a program, assembling it first if given a `.casm` source file. |
...
| `--host-dir <dir>` | `run` | Lets the program open, write, list and remove the host's files under `<dir>`, and nowhere else, through the host file device. See [I/O devices and ports](07-IO-Devices-and-Ports.md#hostfsdevice-0xff0b0000). |
...
| `--cpu-clock <hz>` | `run` | The CPU clock in cycles per second, what turns the machine's cycles into its time (50M by default); `k`, `M` or `G` and an optional `Hz` may follow the number. The timer's `CpuClockHz` and the system control device's report it. |
