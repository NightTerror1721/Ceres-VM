...
| [`libs/devices`](../Ceres/libs/devices) | Terminal, timer, disk, framebuffer, keyboard, mouse and the DMA controller — everything that implements `IODevice`. |
...
| [`devices.h`](../Ceres/libs/devices/include/ceres/devices/devices.h) | The devices that exist today: `SystemControlDevice`, `TimerDevice`, `TerminalDevice`, `DmaController` — plus the keyboard and mouse in `input_devices.h` and the disk and framebuffer in `storage_devices.h`. |
