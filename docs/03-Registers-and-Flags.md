...
| Halting (`HF`) | `1<<5` | `halt` | The `step()` loop, to decide whether to actually fetch/execute or jump the clock to the next device event (or wait for the host) until something happens (see [Interrupts and exceptions](08-Interrupts-and-Exceptions.md#pending-interrupts-and-halt)). |
