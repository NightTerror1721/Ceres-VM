...
namespace ceres::casm
{
	...
	private:
		constexpr OpcodeInfo(Opcode opcode, ParameterArray&& parameters) noexcept :
			_opcode(opcode), _parameters(std::move(parameters))
		{}

	public:
		constexpr Opcode opcode() const noexcept { return _opcode; }
	...
	public:
		static constexpr OpcodeInfo make(Opcode opcode, ParameterArray&& parameters) noexcept
		{
			return OpcodeInfo{ opcode, std::move(parameters) };
		}

		static constexpr OpcodeInfo make(Opcode opcode) noexcept
		{
	...
	public:
		...
		constexpr u32 sizeInBytes() const noexcept { return static_cast<u32>(Instruction::Size * _opcodes.size()); }
		
		inline usize maxOpcodeCountAtAllOverloads() const noexcept
		{
			auto result = findMaxSizeInBytes(_signature.mnemonic);
			if (result.has_value() && *result >= Instruction::Size)
				return *result / Instruction::Size;
			return opcodeCount();
		}
	...
	public:
		...
		static constexpr InstructionInfo make(Signature signature, Opcode opcode) noexcept
		{
