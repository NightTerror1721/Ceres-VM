...
namespace
{
	...
	constexpr u32 TextStart = static_cast<u32>(MemoryMap::UnrestrictedSegmentStartValue);
...
TEST(debugger, restarting_puts_the_machine_back_at_the_entry_point)
{
	...
	CHECK_EQ(session->machine().argumentBlock().count, 1u);                               // and argv[0], the program, is still there
	CHECK(session->isRunning());
}
