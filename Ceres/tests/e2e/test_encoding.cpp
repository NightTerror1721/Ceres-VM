...
TEST(encoding, la_splits_the_address_into_lui_and_ori)
{
	...
	const u32 rodataStart = MemoryMap::UnrestrictedSegmentStart.value() + r.program->header().textSize;
...
TEST(encoding, backward_jump_encodes_a_negative_displacement)
{
	...
	CHECK_EQ(displacement, -static_cast<i32>(Instruction::Size));
}
...
TEST(encoding, forward_call_resolves_to_a_positive_displacement)
{
	...
	CHECK_EQ(displacement, 2 * static_cast<i32>(Instruction::Size));
}
