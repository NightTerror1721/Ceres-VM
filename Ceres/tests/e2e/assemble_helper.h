...
namespace ceres::testing
{
	...
	struct AssembleResult
	{
		...
		std::vector<u32> words() const
		{
			...
			for (usize i = 0; i + Instruction::Size <= text.size(); i += Instruction::Size)
			{
				out.push_back(
		...
		std::string listing() const
		{
			if (!program.has_value())
				return "<no program>";
			return Disassembler::listing(program->text(), MemoryMap::UnrestrictedSegmentStart);
		}
	...
	{
		return std::format("{:08x}  {}", word, Disassembler::disassemble(Instruction(word)));
	}
