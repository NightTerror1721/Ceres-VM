...
namespace
{
	constexpr u32 TextStart = static_cast<u32>(MemoryMap::UnrestrictedSegmentStart.value());
...
TEST(debuginfo, every_instruction_line_round_trips_to_its_own_address)
{
	...
	const DebugInfo& info = assembled.debugInfo;
	CHECK(!info.isEmpty());
...
TEST(debuginfo, symbols_carry_addresses_types_and_constant_values)
{
	...
	const SymbolEntry* main = info.symbolNamed("main");
	CHECK(main != nullptr);
	if (main != nullptr)
	{
		CHECK_EQ(main->kind, static_cast<u8>(SymbolKind::Label));
		CHECK_EQ(main->address, TextStart);
		CHECK_EQ(main->flags & SymbolFlag::Global, u8{ SymbolFlag::Global });
	}

	const SymbolEntry* msg = info.symbolNamed("msg");
	CHECK(msg != nullptr);
	if (msg != nullptr)
	{
		CHECK_EQ(msg->kind, static_cast<u8>(SymbolKind::Variable));
		CHECK_EQ(msg->section, static_cast<u8>(SymbolSection::Rodata));
		CHECK_EQ(msg->scalarType, static_cast<u8>(ScalarType::U8));
	...
	const SymbolEntry* limit = info.symbolNamed("LIMIT");
	CHECK(limit != nullptr);
	if (limit != nullptr)
	{
		CHECK_EQ(limit->kind, static_cast<u8>(SymbolKind::Constant));
		CHECK_EQ(limit->section, static_cast<u8>(SymbolSection::None));
		CHECK_EQ(limit->flags & SymbolFlag::HasValue, u8{ SymbolFlag::HasValue });
...
TEST(debuginfo, the_tables_survive_serialisation)
{
	...
	auto reloaded = DebugInfo::deserialize(bytes);
	...
	const SymbolEntry* limit = reloaded->symbolNamed("LIMIT");
	CHECK(limit != nullptr);
	if (limit != nullptr)
		CHECK_EQ(limit->value, 7u);
...
TEST(debuginfo, a_truncated_or_foreign_debug_section_is_rejected_rather_than_misread)
{
	...
	CHECK(!DebugInfo::deserialize(truncated).has_value());

	std::vector<u8> wrongMagic = bytes;
	wrongMagic[0] = 0x00;
	CHECK(!DebugInfo::deserialize(wrongMagic).has_value());

	CHECK(!DebugInfo::deserialize(std::span<const u8>{}).has_value());
}
...
TEST(debuginfo, a_cres_carries_its_debug_section_through_a_round_trip)
{
	...
	auto info = DebugInfo::deserialize(reloaded->debugSection());
